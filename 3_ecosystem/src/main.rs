...
fn main() {
    ...
    let mut builder = config::Config::builder()
        ...
        .add_source(config::Environment::with_convert_case(
        ...
        .unwrap()
        .set_default("jobs", (num_cpus::get() / 2).to_string())
        .unwrap()
        .set_default("quality", "100.0")
        .unwrap();

    if let Some(pathb) = args.images_file {
        builder = builder
            .set_default("image_list", pathb.display().to_string())
            .unwrap();
    }
...
fn process_image_bytes<B: AsRef<[u8]>>(bytes: B, compress: Compression) -> Result<Vec<u8>, String> {
    ...
    rgba_image
        .write_to(
            &mut compressed_data,
            image::ImageOutputFormat::Jpeg(compress),
        )
...
fn process_file(path: &Path, cfg: &Conf) -> Result<(), String> {
    ...
    let image_data =
        std::fs::read(path).map_err(|_| format!("cannot read file: {}", path.display()))?;
