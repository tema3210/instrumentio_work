        ...
        Ok(pairs) => {
            ...
            for sub in pair.into_inner() {
                match sub.as_rule() {
                    ...
                    Rule::precision => match sub.as_str() {
                        ...
                        i if let Ok(uint) = i.parse::<usize>() => {
                            precision = Some(Precision::Integer(uint))
                        }
...
pub fn parse(input: &str) -> ToGet {
    let sign = {
        ...
        c(input, 1).and_then(|(c, _)| match c {
            "+" => Some(Sign::Plus),
            "-" => Some(Sign::Minus),
            _ => None,
        })
    ...
    let width = width_matcher(input, 1).and_then(|(c, _)| match c {
        w if let Ok(width) = w.parse::<usize>() => Some(width),
        "" => None,
        _ => unreachable!(),
    });

    let precision = precision_matcher(input, 2).and_then(|(c, _)| match c {
        ...
        a if a.starts_with('$')
            && let Ok(arg) = a[1..].parse() =>
        {
            Some(Precision::Argument(arg))
        }
        a if a.starts_with('$') => Some(Precision::ArgumentStr(a[1..].into())),
        _ => None,
    });
