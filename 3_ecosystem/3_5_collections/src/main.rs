...
pub trait UserRepo {
    ...
    //yay
    fn by_ids<'s,S: AsRef<str>>(&'s self, iter: impl Iterator<Item = S> + 's) -> impl Iterator<Item = User> + 's;
...
impl UserRepo for ImmutCollection {
    ...
    fn search<'s,S: AsRef<str> + 's>(&'s self,alike: S) -> impl Iterator<Item = User> + 's {
        self.0.iter().filter_map(move |(_,v)| if v.name.contains(alike.as_ref()) { Some(v.clone()) } else { None })
    }
}
...
mod tests {
    ...
    fn test_by_ids() {
        ...
        assert_eq!(us,vec![User::new("id1","name1"),User::new("id2","name2")]);
    }
    ...
    fn test_search() {
        ...
        assert_eq!(found,HashSet::from([User::new("id1","name1"),User::new("id2","name2")]));
    }
}
